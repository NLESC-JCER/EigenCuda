...
Mat cusolver_diag(Mat A)
{
    ...
    cudaStat1 = cudaMemcpy(d_A, hA, sizeof(float) * lda * m, cudaMemcpyHostToDevice);
    assert(cudaSuccess == cudaStat1);
    ...
    // copy data to host
    cudaStat1 = cudaMemcpy(hW, d_W, sizeof(float)*m, cudaMemcpyDeviceToHost);
    assert(cudaSuccess == cudaStat1);

    //cudaStat2 = cudaMemcpy(hV, d_A, sizeof(float)*lda*m, cudaMemcpyDeviceToHost);
    //assert(cudaSuccess == cudaStat2);
