...
TensorMatrix<T>::tensor_dot_matrix(std::vector<Mat<T>> tensor, Mat<T> B) {
  ...
  cudaMemcpyAsync(dA, _tensorA, size_batch, cudaMemcpyHostToDevice,this -> _stream);
  cudaMemcpyAsync(dB, _tensorB, size_batch, cudaMemcpyHostToDevice,this -> _stream);
  cudaMemcpyAsync(dC, _tensorC, size_batch, cudaMemcpyHostToDevice,this -> _stream);
  ...
  cudaMemcpyAsync(_tensorC, dC, size_batch, cudaMemcpyDeviceToHost, this -> _stream);

  // Copy each array back to the device
  for (auto i = 0; i < _batchCount; i++) {
    T *hout = rs[i].data();
    T *dout = _tensorC[i];
    cudaMemcpyAsync(hout, dout, size_out, cudaMemcpyDeviceToHost, this ->  _stream);
